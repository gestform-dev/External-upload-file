...
def __get_path(key: str, default: Union[PathLike, str]) -> Path:
    """
    Return a normalized, absolute path based on the environment variable
    or a default
    """
    return Path(os.environ.get(key, default)).resolve()


def __get_list(key: str, default: Optional[List[str]] = None,
               sep: str = ",", ) -> List[str]:
    """
...
def _parse_beat_schedule() -> Dict:
    ...
    https://docs.celeryq.dev/en/stable/userguide/periodic-tasks.html#beat
    -entries
    https://docs.celeryq.dev/en/latest/userguide/calling.html#expiration
    """
    schedule = {}
    tasks = [{"name": "Check all e-mail accounts",
              "env_key": "PAPERLESS_EMAIL_TASK_CRON",
              # Default every ten minutes
              "env_default": "*/10 * * * *",
              "task": "paperless_mail.tasks.process_mail_accounts",
              "options": {  # 1 minute before default schedule sends again
                  "expires": 9.0 * 60.0, }, }, {"name": "Train the classifier",
                                                "env_key":
             ...
             {"name": "Optimize the index",
              ...
              "options": {  # 1 hour before default schedule sends again
    ...
    for task in tasks:
        ...
        schedule[task["name"]] = {"task": task["task"],
                                  "schedule": crontab(minute, hour, day_week,
                                                      day_month, month),
...
# OPTION TO DISABLE PARSER ON DOCUMENT CONSUMER
DISABLE_PARSER = True
# OPTION TO RENAME ORIGINAL DOCUMENT WITH CORRELATION_ID
RENAME_ORIGINAL_DOCUMENT_WITH_CORRELATION_ID = True
...
CONSUMPTION_DIR = __get_path("PAPERLESS_CONSUMPTION_DIR",
                             BASE_DIR.parent / "consume", )

# This will be created if it doesn't exist
SCRATCH_DIR = __get_path("PAPERLESS_SCRATCH_DIR",
...
INSTALLED_APPS = ["whitenoise.runserver_nostatic", "django.contrib.auth",
                  "django.contrib.contenttypes", "django.contrib.sessions",
                  "django.contrib.messages", "django.contrib.staticfiles",
                  "corsheaders", "django_extensions", "paperless",
                  "documents.apps.DocumentsConfig",
                  "paperless_tesseract.apps.PaperlessTesseractConfig",
                  "paperless_text.apps.PaperlessTextConfig",
                  "paperless_mail.apps.PaperlessMailConfig",
                  "paperless_kafka.apps.PaperlessKafkaConfig",
                  "paperless_send_email.apps.PaperlessSendEmailConfig",
                  "paperless_send_sms.apps.PaperlessSendSmsConfig",
                  "django.contrib.admin", "rest_framework",
                  "rest_framework.authtoken", "django_filters",
                  "django_celery_results", "guardian", "crispy_forms",
                  "crispy_bootstrap4", "phonenumber_field", *env_apps, ]

CRISPY_TEMPLATE_PACK = 'bootstrap4'

AUTH_USER_MODEL = "paperless.CustomUser"
...
CHANNEL_LAYERS = {
    "default": {"BACKEND": "channels_redis.pubsub.RedisPubSubChannelLayer",
                "CONFIG": {"hosts": [_CHANNELS_REDIS_URL], "capacity": 2000,
                           # default 100
...
GUARDIAN_GET_INIT_ANONYMOUS_USER = ("paperless.models"
...
if AUTO_LOGIN_USERNAME:
    _index = MIDDLEWARE.index("django.contrib.auth.middleware"
...
if ENABLE_HTTP_REMOTE_USER:
    ...
    AUTHENTICATION_BACKENDS.insert(0, ("django.contrib.auth.backends"
                                       ".RemoteUserBackend"))
    REST_FRAMEWORK["DEFAULT_AUTHENTICATION_CLASSES"].append(
...
CORS_ALLOWED_ORIGINS = __get_list("PAPERLESS_CORS_ALLOWED_HOSTS",
                                  ["http://localhost:8000"], )
...
SECURE_PROXY_SSL_HEADER = (tuple(json.loads(os.environ[
                                                "PAPERLESS_PROXY_SSL_HEADER"])) if "PAPERLESS_PROXY_SSL_HEADER" in os.environ else None)
...
AUTH_PASSWORD_VALIDATORS = [{"NAME": ("paperless.validators"
...
def _parse_db_settings() -> Dict:
    ...
    if os.getenv("PAPERLESS_DBHOST"):
        ...
        databases["default"] = {"HOST": os.getenv("PAPERLESS_DBHOST"),
                                "NAME": os.getenv("PAPERLESS_DBNAME",
                                                  "paperless"),
...
LANGUAGES = [("fr-fr", _("French")),  # needs to be first to act as fallback
             # language
             ("en-us", _("English (US)")), ("en-gb", _("English (GB)")), ]
...
LOGGING = {"version": 1, "disable_existing_loggers": False, "formatters": {
    ...
    "simple": {"format": "{levelname} {message}", "style": "{", }, },
           "handlers": {"console": {"level": "DEBUG" if DEBUG else "INFO",
                        ...
                        "file_kafka": {
                            "class":
                                "concurrent_log_handler.ConcurrentRotatingFileHandler",
                            "formatter": "verbose",
                            "filename": os.path.join(LOGGING_DIR, "kafka.log"),
                            "maxBytes": LOGROTATE_MAX_SIZE,
                            "backupCount": LOGROTATE_MAX_BACKUPS, },
                        "file_send_email": {
                            "class":
           ...
           "loggers": {"documents": {"handlers": ["console", "file_documents"],
                                     "level": "DEBUG"},
                       "paperless": {"handlers": ["console", "file_paperless"],
                                     "level": "DEBUG"},
                       "paperless_mail": {"handlers": ["console", "file_mail"],
                                          "level": "DEBUG"},
                       "paperless_kafka": {
                           "handlers": ["console", "file_kafka"],
                           "level": "DEBUG"}, "paperless_send_email": {
                   "handlers": ["console", "file_send_email"],
                   "level": "DEBUG"}, "paperless_send_sms": {
                   "handlers": ["console", "file_send_sms"], "level": "DEBUG"},
                       "paperless_reporting": {
                           "handlers": ["console", "file_reporting"],
                           "level": "DEBUG"}, "paperless_management": {
                   "handlers": ["console", "file_management"],
                   "level": "DEBUG"},
                       "celery": {"handlers": ["console", "file_celery"],
                                  "level": "DEBUG"},
                       "kombu": {"handlers": ["console", "file_celery"],
                                 "level": "DEBUG"}, }, }
...
CELERY_BROKER_TRANSPORT_OPTIONS = {
    "global_keyprefix": os.getenv("HOSTNAME", ""), }

CELERY_TASK_TRACK_STARTED = True
CELERY_TASK_TIME_LIMIT: Final[int] = __get_int("PAPERLESS_WORKER_TIMEOUT",
                                               1800)
...
# https://docs.celeryq.dev/en/stable/userguide/configuration.html#task
# -serializer
CELERY_TASK_SERIALIZER = "pickle"
# https://docs.celeryq.dev/en/stable/userguide/configuration.html#std
...
# https://docs.celeryq.dev/en/stable/userguide/configuration.html#beat
# -schedule-filename
...
THREADS_PER_WORKER = os.getenv("PAPERLESS_THREADS_PER_WORKER",
...
CONSUMER_DELETE_DUPLICATES = __get_boolean("PAPERLESS_CONSUMER_"
                                           "DELETE_DUPLICATES")

CONSUMER_RECURSIVE = __get_boolean("PAPERLESS_CONSUMER_RECURSIVE")

# Ignore glob patterns, relative to PAPERLESS_CONSUMPTION_DIR
CONSUMER_IGNORE_PATTERNS = list(json.loads(
    os.getenv("PAPERLESS_CONSUMER_IGNORE_PATTERNS",
              '[".DS_Store", ".DS_STORE", "._*", ".stfolder/*", '
              '".stversions/*", '
              '".localized/*", "desktop.ini", "@eaDir/*"]',  # noqa: E501
              ), ), )
...
if os.environ.get("PAPERLESS_OCR_MAX_IMAGE_PIXELS") is not None:
    OCR_MAX_IMAGE_PIXELS: int = int(os.environ.get("PAPERLESS_OCR_"
                                                   "MAX_IMAGE_PIXELS"))

# OCR_USER_ARGS = os.getenv("PAPERLESS_OCR_USER_ARGS", "{}")
OCR_USER_ARGS = "{\"invalidate_digital_signatures\": true}"
...
# Maximum number of dates taken from document start to end
# to show as suggestions for
...
FILENAME_FORMAT_REMOVE_NONE = __get_boolean(
    "PAPERLESS_FILENAME_FORMAT_REMOVE_NONE", "NO", )

THUMBNAIL_FONT_NAME = os.getenv("PAPERLESS_THUMBNAIL_FONT_NAME",
...
TIKA_GOTENBERG_ENDPOINT = os.getenv("PAPERLESS_TIKA_GOTENBERG_ENDPOINT",
                                    "http://localhost:3000", )
...
def _parse_ignore_dates(env_ignore: str, date_order: str = DATE_ORDER, ) -> \
    ...
    Args:
        env_ignore (str): The value of the environment variable,
    ...
    for s in env_ignore.split(","):
        d = dateparser.parse(s, settings={"DATE_ORDER": date_order, }, )
        if d:
            ignored_dates.add(d.date())
    return ignored_dates


# List dates that should be ignored when trying to
# parse date from document text
...
TOUR_DISABLED = __get_boolean("TOUR_DISABLED", "true")
TEXT_FORMAT_ENABLED_IN_CONSUMER = __get_boolean("TOUR_DISABLED", "false")
...
def _get_nltk_language_setting(ocr_lang: str) -> Optional[str]:
    ...
    NLTK Languages:
      - https://www.nltk.org/api/nltk.stem.snowball.html#nltk.stem.snowball
        .SnowballStemmer

    """
    ocr_lang = ocr_lang.split("+")[0]
    iso_code_to_nltk = {"dan": "danish", "nld": "dutch", "eng": "english",
...
def _get_sentry_env() -> str:
    ...
    return env if env is not None else 'development'


sentry_sdk.init(
    dsn="",
    integrations=[DjangoIntegration()], environment=_get_sentry_env(),
