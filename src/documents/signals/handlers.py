...
from documents import matching, constants
...
def before_task_publish_handler(sender=None, headers=None, body=None, **kwargs):
    ...
    try:
        task_args = body[0]
        input_doc, doc_metadata = task_args

        task_file_name = input_doc.original_file.name

        PaperlessTask.objects.create(
            user_id=doc_metadata.owner_id,
            task_id=headers["id"],
...
def create_advance_ui_permission(sender=None, **kwargs):
    permission_action = ["add", "view", "change", "delete"]

    content_type = ContentType.objects.get_or_create(
        model=constants.ADVANCE_UI_CONTENT_TYPE,
        defaults={
            "app_label": constants.DOCUMENTS_LABEL,
            "model": constants.ADVANCE_UI_CONTENT_TYPE,
        },
    )

    for action in permission_action:
        codename = "{0}_{1}".format(action, content_type[0].model)
        name = "Can {0} {1}".format(action, constants.ADVANCE_UI_CONTENT_TYPE_VERBOSE)
...
def create_account_settings_permission(sender=None, **kwargs):
    permission_action = ["add", "view", "change", "delete"]

    content_type = ContentType.objects.get_or_create(
        model=constants.ACCOUNT_SETTINGS_CONTENT_TYPE,
        defaults={
            "app_label": constants.DOCUMENTS_LABEL,
            "model": constants.ACCOUNT_SETTINGS_CONTENT_TYPE,
        },
    ...
    for action in permission_action:
        codename = "{0}_{1}".format(action, content_type[0].model)
        name = "Can {0} {1}".format(action, constants.ACCOUNT_SETTINGS_CONTENT_TYPE_VERBOSE)
        Permission.objects.get_or_create(
            codename=codename,
            defaults={
                "name": name,
