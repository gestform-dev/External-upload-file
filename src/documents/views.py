...
from .serialisers import FaqConfigSerializer
...
from paperless_management.user_configs.user_configs_enum import UserConfigsEnum
...
class IndexView(TemplateView):
    ...
    def get_frontend_language(self):
        if (hasattr(self.request.user,
                    "ui_settings", ) and
            self.request.user.ui_settings.settings.get(
                "language")):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["styles_css"] = (
            f"frontend/{self.get_frontend_language()}"
            "/styles.css"
        )
        context["runtime_js"] = (
            f"frontend/{self.get_frontend_language()}"
...
class CorrespondentViewSet(ModelViewSet, PassUserMixin):
    ...
    queryset = Correspondent.objects.annotate(
        ...
        last_correspondence=Max("documents__created"), ).order_by(
        Lower("name"))
...
class DocumentViewSet(PassUserMixin, RetrieveModelMixin, UpdateModelMixin,
    ...
    ordering_fields = (
        "id", "title", "correspondent__name", "document_type__name", "created",
        "modified", "added", "archive_serial_number", "num_notes", "owner",)
    ...
    def original_requested(request):
        return ("original" in request.query_params and request.query_params[
            "original"] == "true")

    def file_response(self, pk, request, disposition):
        doc = Document.objects.get(id=pk)
        if request.user is not None and not has_perms_owner_aware(request.user,
                                                                  "view_document",
                                                                  doc, ):
        ...
        if not self.original_requested(request) and doc.has_archive_version:
            ...
            if mime_type in {"application/csv", "text/csv"} and (
                disposition == "inline"):
        ...
        filename_normalized = normalize("NFKD",
    ...
    def metadata(self, request, pk=None):
        try:
            ...
            if request.user is not None and not has_perms_owner_aware(
                request.user, "view_document", doc, ):
                return HttpResponseForbidden("Insufficient permissions")
        except Document.DoesNotExist:
            raise Http404

        meta = {"original_checksum": doc.checksum,
        ...
        if doc.has_archive_version:
            ...
            meta["archive_metadata"] = self.get_metadata(doc.archive_path,
                                                         "application/pdf", )
        else:
    ...
    def suggestions(self, request, pk=None):
        ...
        if settings.NUMBER_OF_SUGGESTED_DATES > 0:
            ...
            dates = sorted({i for i in itertools.islice(gen,
                                                        settings.NUMBER_OF_SUGGESTED_DATES)}, )

        return Response({"correspondents": [c.id for c in
                                            match_correspondents(doc,
                                                                 classifier,
                                                                 request.user)],
                         "tags": [t.id for t in
                                  match_tags(doc, classifier, request.user)],
                         "document_types": [dt.id for dt in
                                            match_document_types(doc,
                                                                 classifier,
                                                                 request.user)],
    ...
    def getNotes(self, doc):
        return [{"id": c.id, "note": c.note, "created": c.created,
                 "user": {"id": c.user.id, "username": c.user.username,
                          "first_name": c.user.first_name,
                          "last_name": c.user.last_name, }, } for c in
                Note.objects.filter(document=doc).order_by("-created")]
    ...
    def notes(self, request, pk=None):
        ...
        if request.method == "GET":
            try:
                ...
                return Response(
                    {"error": "Error retreiving notes, check logs for more "
                              "detail."}, )
        elif request.method == "POST":
            try:
                c = Note.objects.create(document=doc,
                ...
                return Response(
                    {"error": "Error saving note, check logs for more "
                              "detail.", }, )
...
class GetAllPksView(APIView):
    permission_classes = (IsAuthenticated, PaperlessAdminPermissions)
    pagination_class = StandardPagination()

    def get(self, request, format=None):
        document_ids = Document.objects.values_list('id', flat=True).order_by()
        paginated_data = self.pagination_class.paginate_queryset(document_ids,
                                                                 request)

        return Response({"document_ids": paginated_data})


class SearchResultSerializer(DocumentSerializer, PassUserMixin):
    def to_representation(self, instance):
        doc = Document.objects.get(id=instance["id"])
        notes = ",".join([str(c.note) for c in
                          Note.objects.filter(document=instance["id"])], )
        r = super().to_representation(doc)
        r["__search_hit__"] = {"score": instance.score,
                               "highlights": instance.highlights("content",
                                                                 text=doc.content),
                               "note_highlights": instance.highlights("notes",
                                                                      text=notes) if doc else None,
                               "rank": instance.rank, }
...
class UnifiedSearchViewSet(DocumentViewSet):
    ...
    def _is_search_request(self):
        return (
            "query" in self.request.query_params or "more_like_id" in
    ...
    def filter_queryset(self, queryset):
        if self._is_search_request():
            ...
            return query_class(self.searcher, self.request.query_params,
    ...
    def list(self, request, *args, **kwargs):
        if self._is_search_request():
            ...
            try:
                ...
                logger.warning(f"An error occurred listing search"
                               f"results: {e!s}")
                return HttpResponseBadRequest(("Error listing search results,"
                                               "check logs for more "
                                               "detail."), )
        else:
            return super().list(request)
...
class LogViewSet(ViewSet):
    ...
    def list(self, request, *args, **kwargs):
        exist = [log for log in self.log_files if
                 os.path.isfile(self.get_log_filename(log))]
...
class PostDocumentView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        archive_serial_number = (
            serializer.validated_data.get("archive_serial_number"))
        ...
        input_doc_overrides = DocumentMetadataOverrides(filename=doc_name,
                                                        title=title,
        ...
        async_task = consume_file.delay(input_doc, input_doc_overrides, )
...
class SelectionDataView(GenericAPIView):
    ...
    def post(self, request, format=None):
        ...
        correspondents = Correspondent.objects.annotate(document_count=Count(
            Case(When(documents__id__in=ids, then=1),
                 output_field=IntegerField()), ), )

        tags = Tag.objects.annotate(document_count=Count(
            Case(When(documents__id__in=ids, then=1),
                 output_field=IntegerField()), ), )

        types = DocumentType.objects.annotate(document_count=Count(
            Case(When(documents__id__in=ids, then=1),
                 output_field=IntegerField()), ), )

        storage_paths = StoragePath.objects.annotate(document_count=Count(
            Case(When(documents__id__in=ids, then=1),
                 output_field=IntegerField()), ), )

        r = Response({"selected_correspondents": [
            {"id": t.id, "document_count": t.document_count} for t in
            correspondents],
            "selected_tags": [{"id": t.id, "document_count": t.document_count}
                              for t in tags], "selected_document_types": [
                {"id": t.id, "document_count": t.document_count} for t in
                types], "selected_storage_paths": [
                {"id": t.id, "document_count": t.document_count} for t in
                storage_paths], }, )
...
class SearchAutoCompleteView(APIView):
    ...
    def get(self, request, format=None):
        ...
        return Response(index.autocomplete(ix, term, limit, user, ), )
...
class StatisticsView(APIView):
    ...
    def get(self, request, format=None):
        ...
        documents = (
            Document.objects.all() if user is None else
            get_objects_for_user_owner_aware(
                user, "documents.view_document", Document, ))
        tags = (
            Tag.objects.all() if user is None else
            get_objects_for_user_owner_aware(
                user, "documents.view_tag", Tag))

        documents_total = documents.count()

        inbox_tag = tags.filter(is_inbox_tag=True)

        documents_inbox = (documents.filter(
            tags__is_inbox_tag=True).distinct().count() if inbox_tag.exists(

            ) else None)

        document_file_type_counts = (documents.values("mime_type").annotate(
            mime_type_count=Count("mime_type")).order_by(
            "-mime_type_count") if documents_total > 0 else 0)

        character_count = (
            documents.annotate(characters=Length("content"), ).aggregate(
                Sum("characters")).get("characters__sum"))

        return Response({"documents_total": documents_total,
                         "documents_inbox": documents_inbox,
                         "inbox_tag": inbox_tag.first().pk if
                         inbox_tag.exists() else None,
...
class BulkDownloadView(GenericAPIView):
    ...
    def post(self, request, format=None):
        ...
        follow_filename_format = serializer.validated_data.get("follow"
                                                               "_formatting")

        os.makedirs(settings.SCRATCH_DIR, exist_ok=True)
        temp = tempfile.NamedTemporaryFile(dir=settings.SCRATCH_DIR,
...
class RemoteVersionView(GenericAPIView):
    def get(self, request, format=None):
        ...
        return Response({"version": remote_version,
...
class UiSettingsView(GenericAPIView):
    ...
    def get(self, request, format=None):
        ...
        if settings.TOUR_DISABLED:
            ui_settings["tour_complete"] = True

        user_resp = {"id": user.id, "username": user.username,
                     "phone_number": str(user.phone_number),
                     "is_superuser": user.is_superuser, "groups": list(
        ...
        if user.date_of_birth != "":
            user_resp["date_of_birth"] = user.date_of_birth

        # strip <app_label>.
        roles = map(lambda perm: re.sub(r"^\w+.", "", perm),
                    user.get_all_permissions())
        return Response({"user": user_resp, "settings": ui_settings,
                         "permissions": roles, }, )
...
class FaqConfigView(GenericAPIView):
    permission_classes = (IsAuthenticated,)
    serializer_class = FaqConfigSerializer

    def get(self, request, format=None):
        try:
            user = User.objects.get(pk=request.user.id)
            with open(constants.PREFIX_CONFIG_PATH + user.user_id + ".json",
                      "r") as file:
                json_file = json.load(file)
                faq_data = json_file['faq']
            data_to_send = []
            for entry in faq_data:
                section_to_add = {"question": entry["question"],
                                  "response": entry["response"],
                                  "isVisible": False}
                data_to_send.append(section_to_add)
            return Response({"faq": data_to_send})
        except Exception:
            return Response({
                "faq": [{"question": "", "response": "", "isVisible": False}]})
...
class TasksViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = (PaperlessTask.objects.filter(acknowledged=False,
