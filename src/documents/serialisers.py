...
from paperless_management.document_management import deposit_document_fail_management
...
class OwnedObjectSerializer(serializers.ModelSerializer, SetPermissionsMixin):
    ...
    def get_user_can_change(self, obj):
        checker = ObjectPermissionChecker(
            self.user) if self.user is not None else None
        return (
            obj.owner is None
    ...
    def update(self, instance, validated_data):
        if "created_date" in validated_data and "created" not in validated_data:
            new_datetime = datetime.datetime.combine(
                ...
                datetime.time(0, 0, 0, 0, zoneinfo.ZoneInfo(
                    settings.TIME_ZONE)),
            )
...
class PostDocumentSerializer(serializers.Serializer):
    ...
    def validate_document(self, document):
        ...
        try:
            size = len(document.file.getbuffer())
        except:
            size = 0

        if not is_mime_type_supported(mime_type):
            message = DocumentErrorLabelEnum.MESSAGE_FILE_EXTENSION_NOT_ALLOWED.value
            deposit_document_fail_management(self.user, document.name,
                                             correlation_id=constants.NA_LABEL,
                                             mime_type=mime_type,
                                             file_size_in_bytes=size,
                                             error=message, sendEmail=False)
            raise serializers.ValidationError(
...
class StoragePathSerializer(MatchingModelSerializer, OwnedObjectSerializer):
    ...
    def validate_path(self, path):
        try:
            ...
            raise serializers.ValidationError(
                _("Invalid variable detected.")) from err
...
class FaqConfigSerializer(serializers.ModelSerializer):
    class Meta:
        model = FaqConfig
        depth = 1
        fields = [
