...
class BogusClient:
    ...
    def authenticate(self, mechanism, authobject):
        ...
        if (auth_bytes
                != b"\x00admin\x00w57\xc3\xa4\xc3\xb6\xc3\xbcw4b6huwb6nhu"):
...
class BogusMailBox(ContextManager):
    ...
    def login(self, username, password):
        ...
        # if the password is not ASCII only
    ...
    def fetch(self, criteria, mark_seen, charset=""):
        ...
        # ['NOT', '(X-GM-LABELS', '"processed"']
        if "(X-GM-LABELS" in criteria:
...
class TestMail(
    ...
    def create_message(
        ...
        # TODO: Unsure how to add a uid to the actual EmailMessage.
        # This hacks it in, based on how imap_tools uses regex to extract it.
    ...
    def test_get_correspondent(self):
        ...
        rule = MailRule(
            name="a",
            assign_correspondent_from=(MailRule
                                       .CorrespondentSource.FROM_NOTHING),
        )
    ...
    def test_get_title(self):
        ...
        self.assertEqual(handler._get_title(
            message, att, rule), "this_is_the_file")
        rule = MailRule(
            name="b",
            assign_title_from=MailRule.TitleSource.FROM_SUBJECT,
        )
        self.assertEqual(handler._get_title(
            message, att, rule), "the message title")
    ...
    def test_handle_message(self):
        ...
        self.assert_queue_consumption_tasks_call_args(
            [
                [
                    {"override_title": "file_0",
                     "override_filename": "file_0.pdf"},
                    {"override_title": "file_1",
                     "override_filename": "file_1.pdf"},
                ],
            ],
        )
    ...
    def test_filename_filter_inline_no_consumption(self):
        ...
        WHEN:
            - Given email with inline attachment that does
              not meet filename filter
        THEN:
    ...
    def test_handle_mail_account_tag(self):
        ...
        self.assertEqual(len(self.bogus_mailbox.messages), 3)
        self.assertEqual(len(self.bogus_mailbox.fetch(
            "UNKEYWORD processed", False)), 2)
    ...
    def test_error_skip_mail(self, m):
        ...
        self.assertEqual(len(self.bogus_mailbox.messages), 1)
        self.assertEqual(
            self.bogus_mailbox.messages[0].from_, "amazon@amazon.de")
    ...
    def test_error_create_correspondent(self):
        ...
        with mock.patch(("paperless_mail.mail."
                         "Correspondent.objects.get_or_create")) as m:
    ...
    def test_filters(self):
        ...
        for f_body, f_from, f_to, f_subject, expected_mail_count in [
            ...
            with self.subTest(f_body=f_body, f_from=f_from,
                              f_subject=f_subject):
    ...
    def assert_queue_consumption_tasks_call_args(self,
                                                 expected_call_args: List):
        """
        Verifies that queue_consumption_tasks has been called
        with the expected arguments.

        expected_call_args is the following format:

        * List of calls to queue_consumption_tasks, called once per mail,
          where each element is:
        * List of signatures for the consume_file task, where each element is:
        * dictionary containing arguments that
          need to be present in the consume_file signature.
    ...
    def apply_mail_actions(self):
        """
        Applies pending actions to mails by inspecting calls
        to the queue_consumption_tasks method.
        """
        for args, kwargs in self._queue_consumption_tasks_mock.call_args_list:
            message = kwargs["message"]
            rule = kwargs["rule"]
            apply_mail_action([], rule.pk, message.uid,
...
class TestManagementCommand(TestCase):
    @mock.patch(
        ...
        "mail_fetcher.tasks.process_mail_accounts",
    )
