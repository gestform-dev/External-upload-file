...
export const FILE_STATUS_MESSAGES = {
  ...
  file_extension_not_allowed: () => $localize`File extension not allowed...`,
  file_size_too_large: ({max_size}) =>  $localize`The selected file is too large, maximum allowed size is ${max_size || ''} MB` 
}
...
export class ConsumerStatusService {
  constructor(private readonly permissionsService: PermissionsService) {
    this.currentUser = this.permissionsService.getCurrentUser()
   }
  ...
  connect() {
    ...
    this.statusWebSocket.onmessage = (ev) => {
      ...
      const loggedUserId = this.currentUser.id

      if (loggedUserId && statusMessage.user_id === loggedUserId) {
        ...
        if (
          ...
          status.message = FILE_STATUS_MESSAGES[statusMessage.message](statusMessage?.config_info || undefined)
