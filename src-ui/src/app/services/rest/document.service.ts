...
import { PermissionAction, PermissionType, PermissionsService } from '../permissions.service'
...
export class DocumentService extends AbstractPaperlessService<PaperlessDocument> {
  ...
  DOCUMENT_SORT_FIELDS = this.generateDocumentSortFields();
  DOCUMENT_SORT_FIELDS_FULLTEXT = this.generateDocumentSortFieldsFullText()
  ...
  generateDocumentSortFields() {
    const hasCorrespondentPermission = this.permissionsService.currentUserCan(PermissionAction.View, PermissionType.Correspondent)
    const hasDocumentTypePermission = this.permissionsService.currentUserCan(PermissionAction.View, PermissionType.DocumentType)
    const hasAdvanceUiPermission = this.permissionsService.currentUserCan(PermissionAction.View, PermissionType.AdvanceUi)

    return [
      { field: 'archive_serial_number', name: $localize`ASN`, canDisplayElement: hasAdvanceUiPermission },
      { field: 'correspondent__name', name: $localize`Correspondent`, canDisplayElement: hasCorrespondentPermission },
      { field: 'title', name: $localize`Title`, canDisplayElement: hasAdvanceUiPermission },
      { field: 'document_type__name', name: $localize`Document type`, canDisplayElement: hasDocumentTypePermission },
      { field: 'created', name: $localize`Created`, canDisplayElement: hasAdvanceUiPermission },
      { field: 'added', name: $localize`Added`, canDisplayElement: true },
      { field: 'modified', name: $localize`Modified`, canDisplayElement: hasAdvanceUiPermission },
      { field: 'num_notes', name: $localize`Notes`, canDisplayElement: hasAdvanceUiPermission },
      { field: 'owner', name: $localize`Owner`, canDisplayElement: hasAdvanceUiPermission },
    ].filter(el => el.canDisplayElement)
  }
  generateDocumentSortFieldsFullText() {
    return [
      ...this.DOCUMENT_SORT_FIELDS,
      {
        field: 'score',
